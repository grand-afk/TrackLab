...
export function StemRow({ stem, audioBuffer, wsRef, isFirst }: Props) {
  ...
  return (
    <div className="border-b border-zinc-800 last:border-0">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={cn(
              ...
              stem.muted ? 'text-red-400 bg-red-500/10' : 'text-zinc-400 hover:text-zinc-200'
            )}
          ...
          <button
            ...
            className={cn(
              'p-1.5 rounded text-xs font-bold transition-colors',
      ...
      {/* Waveform */}
      <div className="px-0">
        <Waveform stem={stem} wsRef={wsRef} />
      </div>
      ...
      {showSpec && !audioBuffer && (
        <div className="flex items-center gap-2 px-3 py-2 text-xs text-zinc-500">
          <Loader2 className="w-3 h-3 animate-spin" /> Loading spectrogram…
        </div>
      )}
    </div>
  )
